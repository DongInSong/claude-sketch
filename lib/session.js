...
// run to hundreds of MB, mostly attachment and text payloads. A typed prompt and
// a queued one carry the label, so they have to come through too: left out, the
// title was read off last-prompt records alone, which lag a turn behind the
// question actually asked — so the panel's label took turns with the /sessions
// poll, which did read the prompt, flicking from the newest question to the one
// before and back.
const INTERESTING = /"tool_use"|"tool_result"|"usage"|"lastPrompt"|"aiTitle"|"summary"|"queue-operation"|"type":"user"/;
...
export class Session {
  constructor({ id, dir, root }) {
    ...
    this.lastScan = 0;
    this.dirty = new Set();    // files a watch named as changed, since the last scan
    this.lastFull = 0;         // when the whole tree was last walked
  }
  ...
  watch() {
    const wake = () => {
      ...
      this.wakeT = setTimeout(() => {
        ...
        this.tick(true);                // a wake knows what moved; the poll does not
  ...
  addWatch(dir, opts) {
    try {
      // The watch says which file changed and that used to be thrown away, so
      // every wake walked the whole subagent tree and stat'd every transcript in
      // it — finished ones included. That scales with what a session has ever
      // spawned rather than what is moving: measured, 1.44ms a scan at nine
      // subagents and 1.80ms at eighteen, several times a second, against the
      // same directory Claude Code is writing to. Named, the busy path is one
      // file.
      const w = fs.watch(dir, opts || {}, (_ev, name) => {
        if (name) this.dirty.add(path.join(dir, String(name)));
        this.wake();
      });
  ...
  // The transcript a path belongs to, or null if it is not one we tail. Nested,
  // because workflow agents live in subagents/workflows/wf_x/ and their meta
  // sits beside them.
  agentAt(fp) {
    const base = path.basename(fp);
    if (!base.startsWith('agent-') || !base.endsWith('.jsonl')) return null;
    return this.label(base.slice(6, -6), path.dirname(fp));
  }

  // Everything a watch named since the last scan, and nothing else.
  //
  // Falls back to the full walk the moment it is handed something it cannot
  // place — a new nested directory, say, which is how a fresh workflow agent
  // first shows up. Discovery has to stay as quick as it was: every one of nine
  // subagents was picked up within 166ms of its first call, and trading that
  // away for a cheaper scan would be paying in the only currency this panel has.
  scanDirty() {
    const named = [...this.dirty];
    this.dirty.clear();
    const jobs = [];
    for (const fp of named) {
      if (fp === this.mainPath) { jobs.push([fp, 'main']); continue; }
      if (path.dirname(fp) === this.dir) continue;      // another session's transcript
      const agent = this.agentAt(fp);
      if (!agent) return this.scan();                   // something new: walk it all
      jobs.push([fp, agent]);
    }
    this.budget = BUDGET;
    this.behind = false;
    for (const [fp, agent] of jobs) this.tailFile(fp, agent);
  ...
  scan() {
    this.budget = BUDGET;
    this.behind = false;
    this.dirty.clear();                 // the walk below covers whatever was named
    ...
    catch { this.lastScan = this.lastFull = Date.now(); return; }
    ...
    this.lastScan = this.lastFull = Date.now();
  }
  ...
  //
  // A tick the watches asked for reads only what they named; the poll always
  // walks the whole tree. So the cheap path only ever runs between full walks —
  // a watch that silently delivers nothing, which is how they behave on some
  // network mounts and container filesystems, costs nothing here, because the
  // poll it was only ever a shortcut for still comes round.
  tick(fromWake) {
    if (this.timer) clearTimeout(this.timer);
    this.timer = null;
    this.quiet++;
    if (fromWake && this.subWatched && this.dirty.size && Date.now() - this.lastFull < POLL_MS)
      this.scanDirty();
    else
      this.scan();
    if (!this.started) return;
