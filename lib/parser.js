...
const ACT = {
  ...
  // Workflow is not listed: its op, flow, is already the verb the doodle knows —
  // colour, glyph and word in four languages. A finer 'orchestrate' matched none
  // of them, so a running workflow drew a blank chit captioned in English.
};
...
// Does this user-channel record owe a reply? Not the same question as "is it a
// title", and the two were answered by one test — cleanPrompt — which reads both
// kinds of wordless record as silence.
//
// They mean opposite things. A slash-command echo (<command-name>,
// <local-command-stdout>) is written into sessions that are long finished, so
// taking it for a turn left dead sessions working for ever. A
// <task-notification> is the harness handing a finished background task back:
// nothing of the user's is in it, and the assistant is about to answer it. Read
// as silence, the whole synthesis that follows — the model reading the results
// and writing them up, tool calls or no — was idle time, and the session it
// belongs to said "done" while it was still working.
const WAKES = /<(task-notification|teammate-message)\b/;
export const owesReply = (txt) => !!cleanPrompt(txt) || WAKES.test(String(txt || ''));
...
export class SessionParser {
  ...
  parseLine(line, agent) {
    ...
    if (line.type === 'user' && typeof msg.content === 'string') {
      ...
      // A task-notification cleans to nothing too and is the opposite: work
      // landing back on the assistant's desk. owesReply tells them apart.
      if (owesReply(msg.content)) out.push({ t: 'open', agent, ts });
      return out;
    }
    if (line.type === 'user' && Array.isArray(msg.content)) {
      ...
      if (!line.isMeta && !msg.content.some(c => c && c.type === 'tool_result')) {
        ...
        if (text) {
          ...
          if (owesReply(text)) out.push({ t: 'open', agent, ts });
        }
      }
