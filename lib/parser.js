...
export class SessionParser {
  constructor(root) {
    ...
    this.usageByMsg = new Map();     // message.id -> {agent, tin, tout}
    this.totals = new Map();         // agent -> {tin, tout}
  ...
  rel(p) {
    ...
    return p.startsWith(this.root) ? p.slice(this.root.length) : p;
  }
  ...
  parseLine(line, agent) {
    ...
    if (line.type === 'last-prompt' && line.lastPrompt) {
      const c = cleanPrompt(line.lastPrompt);
      if (c) { this.prompt = c; this.retitle(out); }
      return out;
    }
    ...
    if (line.type === 'assistant') {
      ...
      // message id — keep the latest per id, then re-sum the agent total.
      if (msg.usage && msg.id) {
        const fresh = msg.usage.input_tokens || 0;
        const cw = msg.usage.cache_creation_input_tokens || 0;
        const cr = msg.usage.cache_read_input_tokens || 0;
        this.usageByMsg.set(msg.id, { agent, fresh, cw, cr, tout: msg.usage.output_tokens || 0 });
        const sum = { fresh: 0, cw: 0, cr: 0, tout: 0 };
        for (const u of this.usageByMsg.values()) {
          if (u.agent !== agent) continue;
          sum.fresh += u.fresh; sum.cw += u.cw; sum.cr += u.cr; sum.tout += u.tout;
        }
        const tin = sum.fresh + sum.cw + sum.cr;
        const prev = this.totals.get(agent);
        if (!prev || prev.tin !== tin || prev.tout !== sum.tout) {
          this.totals.set(agent, { tin, tout: sum.tout });
          out.push({ t: 'usage', agent, tin, tout: sum.tout,
            fresh: sum.fresh, cw: sum.cw, cr: sum.cr });
        }
      }
    ...
    if (line.type === 'user' && typeof msg.content === 'string') {
      const c = cleanPrompt(msg.content);
      if (c) { this.prompt = c; this.retitle(out); }
      return out;
    }
    if (line.type === 'user' && Array.isArray(msg.content)) {
      for (const c of msg.content) {
        ...
        const ev = { t: 'res', id: c.tool_use_id, len, ts };
