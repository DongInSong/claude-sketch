...
export class Project {
  ...
  list() {
    ...
    return {
      ...
      sessions: files.map(f => ({
        id: f.id,
        title: this.title(f) || f.id.slice(0, 8),
        mtimeMs: f.mtimeMs,
        size: f.size,
        active: now - f.mtimeMs < 45 * 1000,   // matches the client's idea of "working"
      })),
    };
  }

  // Titles only move when the file does, and this list is polled — so key the
  // work on (size, mtime) instead of re-reading every ten seconds.
  title(f) {
    if (!this._titles) this._titles = new Map();
    const hit = this._titles.get(f.fp);
    if (hit && hit.size === f.size && hit.mtimeMs === f.mtimeMs) return hit.title;
    const title = this.peekTitle(f.fp);
    this._titles.set(f.fp, { size: f.size, mtimeMs: f.mtimeMs, title });
    if (this._titles.size > 200) this._titles.delete(this._titles.keys().next().value);
    return title;
  }
  ...
  async buildUniverse() {
    ...
    const out = await new Promise((resolve) => {
      execFile('git', ['-C', this.root, 'ls-files'],
