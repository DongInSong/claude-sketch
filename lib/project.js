...
//
// Only the head is read: a session that changes tree does it once, near the
// start, and this is the fallback for a value that is usually right anyway.
...
function workedIn(fp, fallback) {
  ...
  try {
    const fd = fs.openSync(fp, 'r');
    try {
      const buf = Buffer.alloc(Math.min(WORK_SCAN, fs.statSync(fp).size));
      fs.readSync(fd, buf, 0, buf.length, 0);
  ...
  // directory-based vote could never have fired. One git call per distinct
  // directory, memoised, a few dozen at most and only once per session.
  const memo = new Map();
  const perRepo = new Map();
  let total = 0;
  for (const [d, n] of perDir) {
    if (!memo.has(d)) memo.set(d, repoRoot(d));
    const r = memo.get(d);
    perRepo.set(r, (perRepo.get(r) || 0) + n);
    total += n;
  }
  let best = null, bestN = 0;
  for (const [r, n] of perRepo) if (n > bestN) { bestN = n; best = r; }

  // it has to be most of the work, somewhere else, and an actual repository —
  // a scratch directory is where files were written, not where the project is
  if (!best || best === fallback || bestN / total < 0.5) return fallback;
  return fs.existsSync(path.join(best, '.git')) ? best : fallback;
}
