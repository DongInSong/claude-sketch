...
import { cleanPrompt, owesReply } from './parser.js';
...
// A session killed mid-call — or mid-think — leaves its turn open for ever, so
// the signal needs an end. Longer than any real tool run (the worst measured
// here was 427s) or think, short enough that yesterday's crash is not still
// "working".
const OPEN_CALL_MS = 30 * 60 * 1000;

// The tail window both signals are read from. Nothing is written to a transcript
// while a tool runs or the model thinks, so the outstanding call and the last
// turn event are always the last thing in the file.
const TAIL = 256 * 1024;

// How stale a main transcript can be before its subagents are not worth looking
// for. Agents are spawned by main, so nothing under it can outlive main's last
// write by more than one fan-out — and the longest measured here is under two
// hours. Half a day is the difference between a session that might still be
// going and one that is over; past it, the readdir is skipped.
const SUB_WINDOW = 12 * 60 * 60 * 1000;

// At most this many agent tails per session per poll. A session here has 351 of
// them, and the answer wanted is "is any one of them working" — so the newest go
// first and the first yes ends it.
const SUB_PEEK = 24;

// Whose turn is it? The assistant's turn is open — it owes output, so it is
// working — even while the file sits perfectly still through a long think or a
// streamed answer: from the moment a prompt or a tool result lands until it
// writes a message that ends the turn. mtime alone cannot tell, and read both
// ends wrong — a long think as idle (nothing moved the file) and a turn that
// had only just finished as busy for a further 45s (the end_turn was the move).
// The last turn line settles it. last-prompt/ai-title/summary are re-emitted
// every turn and carry no turn; the real last turn event sits at the file's end.
function turnOpen(lines) {
  let open = false;
  for (const ln of lines) {
    if (ln.length < 24) continue;
    if (!ln.includes('"type":"assistant"') && !ln.includes('"type":"user"')
        && !ln.includes('"type":"queue-operation"')) continue;
    let o;
    try { o = JSON.parse(ln); } catch { continue; }
    // Slash-command records ride in on the user role — a local-command caveat
    // (isMeta), then <command-name>/<local-command-stdout> lines whose text is
    // pure wrapper. Claude Code writes them into a session that is long finished,
    // with a fresh mtime, and taking them for a turn left dead sessions "working"
    // for ever. A real turn is a tool result, a real prompt, or an assistant line.
    // owesReply, not cleanPrompt: a <task-notification> is wordless too and means
    // the opposite — a background task coming back, which the assistant answers.
    if (o.isMeta) continue;
    if (o.type === 'assistant' && o.message) open = o.message.stop_reason !== 'end_turn';
    else if (o.type === 'user' && o.message) {
      const c = o.message.content;
      if (Array.isArray(c)) open = true;                            // a tool result or a block message
      else if (typeof c === 'string' && owesReply(c)) open = true;   // a prompt, or work handed back
    }
    else if (o.type === 'queue-operation' && typeof o.content === 'string' && cleanPrompt(o.content)) open = true;
  }
  return open;
}

// quiet is how long an open turn on a still file may go on meaning "working"; a
// call that is genuinely still out always gets OPEN_CALL_MS, because that is what
// an outstanding call is for. Agents want a much shorter one — see AGENT_QUIET.
function working(now, mtimeMs, tail, quiet = OPEN_CALL_MS) {
  // A tool call that went out and never came back — a long Bash, a running
  // subagent — writes nothing while it runs; the outstanding call is its only
  // trace, and it is the assistant at work.
  if (tail.openSince && now - tail.openSince < OPEN_CALL_MS) return true;
  // The turn is open — a reply is owed — but the file has gone quiet: a long
  // think, or an answer streaming out with nothing yet committed. Trust it until
  // the quiet is long enough to mean the session was abandoned, not thinking.
  return !!tail.turnOpen && now - mtimeMs < quiet;
}

// An agent's transcript often just stops. Measured over 2267 of them here, 32%
// end with the turn still open — 55 of one 300-file sample end on the
// StructuredOutput call that *is* the agent's return value, others on a partial
// final line — so "the turn is open" never expires by itself the way main's does,
// and given main's 30 minutes it would have held a session "live" for half an
// hour after its fan-out was over. Only 2 of the 2267 leave a call genuinely
// outstanding, which is the case that keeps the full window.
//
// So an open turn on an agent's file is trusted only while the file is fresh.
// Measured across 84,759 quiet stretches with no call out — an agent thinking or
// streaming, the only thing this branch has to cover — p99 is 82s and p99.9 is
// 345s. Five minutes covers what an agent really does between writes.
const AGENT_QUIET = 5 * 60 * 1000;
...
export class Project {
  ...
  list() {
    ...
    const sessions = files.map(f => {
      const p = this.peek(f);
      const subs = this.subWork(f.id, f.mtimeMs, now);
      // Two answers to "is it working": main's own, and the session's. They part
      // company for the whole of a fan-out, which is why both are reported — main's
      // row says "idle" honestly while the badge on the same session says "live".
      const mainActive = working(now, f.mtimeMs, p);
      return {
        id: f.id,
        title: p.title || f.id.slice(0, 8),
        // an agent writing is the session moving, so it counts for the clock the
        // list is ordered and aged by as much as main's own writing does
        mtimeMs: Math.max(f.mtimeMs, subs.mtimeMs),
        size: f.size,
        active: mainActive || subs.active,
        mainActive,
      };
    });
    sessions.sort((a, b) => b.mtimeMs - a.mtimeMs);
    return { dir: this.dir, sessions };
  }

  // A session that has handed its work to background agents is not working —
  // main's turn is closed, it owes nothing until the results come back — but it is
  // not done either. Main writes nothing at all while a fan-out runs: measured on
  // this machine, one main transcript sat still for 92 minutes with agents writing
  // throughout, so judged on main alone it read "done · 1h ago" at its busiest.
  // The agents' own transcripts are the missing signal, and they answer the same
  // question in the same words — a turn owed, or a call still out.
  //
  // Narrowed twice, because reading every agent transcript on every poll would be
  // absurd at 351 of them: only a file that moved lately can hold a working
  // agent, and the tails are cached by (size, mtime), so a finished agent's file
  // is read once and never again while the ones that come back are exactly the
  // ones still going.
  subWork(id, mainMtimeMs, now) {
    const out = { mtimeMs: 0, active: false };
    if (!/^[A-Za-z0-9-]+$/.test(id) || now - mainMtimeMs > SUB_WINDOW) return out;
    const subRoot = path.join(this.dir, id, 'subagents');
    let names = [];
    try { names = fs.readdirSync(subRoot, { recursive: true }); } catch { return out; }
    const fresh = [];
    for (const n of names) {
      ...
      if (st.mtimeMs > out.mtimeMs) out.mtimeMs = st.mtimeMs;
      // quiet longer than a turn may be left open is dead by the same rule main
      // is judged by, and not worth a read
      if (now - st.mtimeMs < OPEN_CALL_MS) fresh.push({ fp, st });
    }
    fresh.sort((a, b) => b.st.mtimeMs - a.st.mtimeMs);
    for (const f of fresh.slice(0, SUB_PEEK))
      if (working(now, f.st.mtimeMs, this.peekWork(f.fp, f.st), AGENT_QUIET)) { out.active = true; break; }
    return out;
  }

  // The two working signals for a file whose title we do not want. A subagent's
  // transcript opens with the brief main gave it — a real user-role message, and
  // main's instruction to a worker, not the session's name — so peekTail's hunt
  // for a title is not only wasted here, it would find the wrong thing.
  peekWork(fp, st) {
    if (!this._work) this._work = new Map();
    const hit = this._work.get(fp);
    if (hit && hit.size === st.size && hit.mtimeMs === st.mtimeMs) return hit;
    const rec = { size: st.size, mtimeMs: st.mtimeMs, openSince: 0, turnOpen: false };
    try {
      const fd = fs.openSync(fp, 'r');
      try {
        ...
        rec.openSince = openCallSince(lines);
        rec.turnOpen = turnOpen(lines);
      } finally { fs.closeSync(fd); }
    } catch { /* gone, or unreadable: it is not working */ }
    this._work.set(fp, rec);
    if (this._work.size > 400) this._work.delete(this._work.keys().next().value);
    return rec;
  }
  ...
  peekTail(fp) {
    let openSince = 0, open = false;
    try {
      const st = fs.statSync(fp);
      const fd = fs.openSync(fp, 'r');
      try {
        // Same rules as the parser reads over the whole transcript — string and
        // image prompts, queued ones, isMeta skipped — so the list title and the
        // one shown after a click never disagree. The one thing the parser has
        // that this cannot is the whole file: the newest prompt can sit buried a
        // little past the first window, under the output that followed it, so a
        // window holding only last-prompt records is read again, wider.
        let best = null, aiTitle = null, fallback = null;
        for (const size of [TAIL, 4 * 1024 * 1024]) {
          ...
          // An outstanding call and the last turn event both sit at the very end
          // of the file — nothing is written while a tool runs or the model
          // thinks — so the first window always holds them.
          if (size === TAIL) { openSince = openCallSince(lines); open = turnOpen(lines); }

          best = null; aiTitle = null; fallback = null;   // the wider window is a superset — rescan it whole
          for (const ln of lines) {
            if (ln.length < 24) continue;
            if (!ln.includes('"content":"') && !ln.includes('"role":"user"')
                && !ln.includes('"lastPrompt"') && !ln.includes('"aiTitle"')) continue;
            let o;
            try { o = JSON.parse(ln); } catch { continue; }
            if (o.isMeta) continue;              // a caveat, a skill's brief: not a title
            const ts = Date.parse(o.timestamp || '') || 0;
            let text = null;
            if (o.type === 'queue-operation' && typeof o.content === 'string') text = o.content;
              ...
              else if (Array.isArray(mc) && !mc.some(b => b && b.type === 'tool_result'))
                text = mc.filter(b => b && b.type === 'text' && typeof b.text === 'string').map(b => b.text).join(' ');
            }
            else if (o.type === 'ai-title' && o.aiTitle) aiTitle = o.aiTitle;
            else if (o.type === 'last-prompt' && o.lastPrompt) fallback = o.lastPrompt;
            // Clean here, not at the end: a slash-command record (<command-name>…)
            // cleans to nothing, and keeping it as the newest raw line let it hide
            // the real prompt underneath — the panel then fell back to the id.
            if (text && ts) { const c = cleanPrompt(text); if (c && (!best || ts >= best.ts)) best = { ts, text: c }; }
          }
          if (best || from === 0) break;   // a timestamped prompt, or the whole file already read
        }
        const title = best ? best.text : cleanPrompt(fallback || aiTitle);
        if (title) return { title, openSince, turnOpen: open };
      } finally { fs.closeSync(fd); }
    } catch { /* ignore */ }
    return { title: null, openSince, turnOpen: open };
  }
