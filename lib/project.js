...
function turnOpen(lines) {
  ...
  for (const ln of lines) {
    ...
    if (o.type === 'assistant' && o.message) open = o.message.stop_reason !== 'end_turn';
      ...
      else if (typeof c === 'string' && cleanPrompt(c)) open = true;  // a real typed prompt, not a command record
    }
...
function working(now, mtimeMs, tail) {
  ...
  return !!tail.turnOpen && now - mtimeMs < OPEN_CALL_MS;
}
    ...
    return {
      ...
      sessions: files.map(f => {
        ...
        return {
          ...
          mtimeMs: f.mtimeMs,
          size: f.size,
          active: working(now, f.mtimeMs, p),
        };
      }),
    };
  }
    ...
    try {
      ...
      try {
        ...
        for (const size of [262144, 4 * 1024 * 1024]) {
          ...
          if (size === 262144) { openSince = openCallSince(lines); open = turnOpen(lines); }
